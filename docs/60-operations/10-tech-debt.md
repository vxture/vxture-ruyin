...
| TD-010 | 守护进程的原生模块 `better-sqlite3-multiple-ciphers` 按**宿主 Node** 的 ABI 取预编译包，但守护进程经 `utilityProcess.fork` 跑在 **Electron 内置 Node**（electron 42 → ABI 146；CI 的 Node 22 → 127）里；首次 `new Database()` 时 `require` 绑定会报 "compiled against a different Node.js version"。shell smoke 只探 /health，绑定是懒加载，故未暴露 | `scripts/release/pack.mjs` 的 `pnpm deploy` 在系统 Node 下跑 prebuild-install，未指定 `--runtime=electron --target=<electron 版本>`；electron-builder 的 npmRebuild/nodeGypRebuild 因与 pnpm workspace 冲突被关（apps/shell/electron-builder.yml） | pack.mjs 部署守护进程后以 `npm_config_runtime=electron npm_config_target=$(electron --version)` 重取预编译包（上游 v12.11.1 提供 electron-v146 win32-x64），并把"打开一个工作空间并建库"纳入打包冒烟；开发态同样需一份 electron ABI 绑定 | open |
