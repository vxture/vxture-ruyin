...
- **包内引用一律用相对路径**（`app.js`，不是 `/app.js`）：本地把包装在
  `<产品>.localhost:<端口>/<sha256>/` 底下，云端装在别的位置 —— 相对路径两边都对，
  以 `/` 开头的路径只在其中一边对。
...
> **`archive` / `restore` 在运行时里是什么（2026-09-11 落地）。** 只有契约声明了
> 的才做 —— 容器能做什么由产品定，Runtime 不替产品加它没说过的操作；声明了 `archive`
> 没声明 `restore` 的，归档是单向的。
>
> - **归档 = 只读**：发起任务、推进阶段、改授权与绑定、改工具策略、收文件一律被拒
>   （内核 `ProjectArchivedError`，宿主自己管的那部分由守护进程一并拦）。**看与导出
>   照常** —— §18.5 的底线「任何状态下本地数据可访问、可导出」不因归档打折；
> - **前提**：没有还没落定的任务（跑到一半的任务在只读的项目里谁都推不动、也取消不了）；
> - **连带**：作废这个项目名下的产品级凭据（桥面 401、事件流被收掉），撤掉产品挂着的
>   推进请求，产品界面不再装入；侧栏里它离开「最近工作」、另列「已归档」；
> - **恢复**：清掉归档标记，一切照旧。归档与恢复都进审计（`project.archived` /
>   `project.restored`）。
...
- `cloud`（2026-09-15，ADR-026 §5）：**只指经 Runos 调用的云端能力或资产**，不指云端存储 —— 智能体数据不出本机
...
> **既有项目怎么跟上（2026-09-12，ADR-024）。** 产品库的生效版本比项目的快照新，项目
> **直接升**上去（等项目里的任务落定、归档的不升）。上表的规则由 `compareContracts`
> 执行 —— **看实际差异，不看版本号标签**：一个标成 minor 却删了（或改窄了）项目在用的
> 东西的版本，对这个项目就不升，项目留在旧版并在概览里写明原因。想让老项目跟上，就只增
> 不删；删了东西的版本只能给新项目用（major 迁移声明的格式未定，§19）。
