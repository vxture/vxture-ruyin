...
- 状态：**提议**（owner 2026-09-05：「检查 Runos 代码与文档，分析两个能力提供平台
  和机制」）
- 日期：2026-09-05
...
1. **技能来源加「产品分发」层**（§3 c，§4.1）—— 同意与否。同意则 ADR-018 v2.2 改写，
   并给产品接入指南加「能力面转发 Runos 技能目录」一节。
2. **第三方密钥归 Runos 保险库**（§4.2）—— 同意与否。同意则 ADR-018 预置清单里
   「需密钥」一档改为「经 Runos 注册」，Ruyin 本地不做这类连接器。
3. **`scripts/` 的 Runos Executor 出路**（§4.3）只登记不启用 —— 同意与否。
4. Runos 至今没有第一个消费者（Runos ADR-014）。**bid 产品的云端能力面**是不是那个
   第一消费者 —— 这是产品侧的决定，但它决定 §3 c 什么时候能真的接通。
