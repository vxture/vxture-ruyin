...
### 6.2 追加（owner，2026-09-15）：本机保有一份与平台 Runos 能力目录一致的能力**清单** —— 清单，不是安装

owner 定：**Ruyin 本地要有一份能力列表，与平台的 Runos 能力目录保持一致；它是列表，
不是本地安装 —— 能力不必装到本机。**

目录的量级（给「一致」一个量级，不是给它一个定数）：Runos 预置台账
`deploy/preset/ledger.json` 877 个 `capabilityId`（vxture-runos `bd26817a`）；生产
`GET /capability/capabilities` 2026-09-13 实测 886 行（vxture-platform#306）；owner 口述
约 878。三个数互不相等，这本身就说明：**「一致」只能定义为「与平台端点返回的那份
一致」，不能定义为某个写死的条数**（对账口径同 ADR-018 §7.4：不断言两边相等）。

#### 与 §7 的关系：改的是哪半句，留的是哪半句

§7 第二行否掉的是「**把本机技能 / 工具登记册整个换成** Runos 目录的镜像」，理由三条：
Runos 是分发面不是执行面；本机还有预置、用户、项目三层来源；离线必须可用。§6.1 又把
它精确成「换的是取字节的地方，不是取字节的时机」。

- **登记册那一半不变。** 本机登记册仍是「本机可运行的子集」（§2 目录 / 登记一行）：
  四层来源、近者优先、随包预置、运行时不依赖 Runos 可达（§6.1 第 3 句），一个字不改。
  那三条理由仍然成立 —— 也正因为它们成立，清单才必须是**另一样东西**，不能并进登记册。
- **被改的是它顺带定下的另一半：「Ruyin 不需要看见 Runos 目录的全貌」。** 此前本机只知
  道自己能跑的子集，目录全貌只在平台 / Runos 那一侧；现在 Ruyin 本地要持有一份与目录
  一致的列表。就「列表」这一层而言，§7 那句「不镜像」不再成立。

一句话：**清单是目录的投影，登记册是本机可运行的子集；两者并存，前者不喂后者。**

#### 范围：要一致的是什么

| 要一致的（清单需要的） | 说明 |
|---|---|
| `capability_id` | 身份键；与契约 `tools[]`、本机登记册按 id 对得上 |
| `title` / `display_name` | 展示 |
| `primitive_type` | Connector / Skill / Executor / Asset 之分 |
| `category` / `tags` | 分组与筛选 |
| 条目总数；（平台若给）目录版本或 ETag | 判断「是否仍一致」要有依据，不能靠本地猜 |

**不在清单里**：`inputSchema` 等完整契约（那是 `runos_resolve` 的事，沿调用路径按需取）、
技能正文与 `scripts/`、凭证需求、端点实例。

#### 不变的（本条**不是**这些的松动）

| | 仍然是 |
|---|---|
| 谁调用、怎么到达 | §3 不变：Runos 的能力经产品云端 / 平台换票到达，**Ruyin 不直连 Runos**（ADR-001 / 009；平台在 vxture-platform#264 的答复：公共客户端不持任何下游令牌，由平台代换票）。有了清单不等于多了一条调用路径 |
| 装不装 | **不装**。清单条目不触发任何下载、不进登记册、不占 `resources/`；本机能跑什么仍由登记册四层来源决定 |
| 权益判定 | **Ruyin 不判**。清单不带、也不用来推断「这个用户能不能用这个能力」；门控只到产品级（ADR-006），档位不是 Ruyin 的开关（ADR-006 更正节） |
| 配额 / 计量 | 不门控、不计量（ADR-006） |
| 客户端零秘密 | 不变；清单经用户会话取，不需要任何 S2S 凭证 |

#### 权威与取法

- **权威是平台。** 本仓不自建、不手抄、不从构建产物推导这份清单；本地那份只是平台
  那份的缓存。
- **取法必须是桌面会话够得着的平台端点。** 桌面登录自 vxture-platform #297
  （`331498c2`）起走 console-bff 原生握手，读请求带 `X-Vxture-Session`。**这样的端点今天
  不存在**（vxture-platform `9b814c97`、vxture-runos `bd26817a`，2026-09-15 读码）：
  - console-bff 里没有任何路由列 Runos 能力（`bff/console-bff/src` 搜 runos / registry /
    catalog 无结果；`/api/capabilities` 返回的是治理权限码，不是能力目录）；
  - Runos 管理面 `GET /capability/capabilities`（`service/src/registry/registry.controller.ts:47-77`）
    走 `OperatorAuthGuard`（workforce 运营者，`mgmt:runos`）；消费面 `runos_discover`
    （`POST /v1/mcp`）要 S2S `aud=runos`（`gateway/s2s-auth.guard.ts:50-137`）—— 桌面 public
    client 按 #264 的答复永不持有；
  - opera-bff / admin-bff 的代理（`bff/opera-bff/src/routers/runos.router.ts:439,568,607`）
    只对运营者开。
- 请平台补的形状（会话鉴权、cursor 分页、`total`、上表字段、可选版本 / ETag）写在
  `80-liaison/70-2609151351-ruyin-desktop-session-interface-gaps.md` §2 C；本仓登记为 TD-065。

#### 由既有口径直接推出的（不是新决定）

- **清单拿不到不得阻塞执行。** §6.1 第 3 句「运行时永不依赖 Runos 可达」照旧：执行走
  登记册与调用路径，与清单无关。
- **拿不到就说拿不到。** 与 ADR-006 修订节对配额展示的姿态相同：展示的是平台那份事实
  的镜子；取不到时如实标注快照时间或「未取到」，不用旧列表装作仍是最新。

#### 待定（不在本条范围，实现前要定）

1. 清单在界面上放哪、怎么呈现（能力平台分区 ADR-018 §2.7 里新增一格，还是别处）。
2. 刷新时机（登录后 / 定时 / 按版本比对）—— 取决于平台端点给不给版本或 ETag。
3. 清单条目与本机登记册条目要不要对照标记（「目录里有、本机可运行」）。

owner 未就这三件表态，本条不预设。

### 6.3 追加（owner，2026-09-15）：清单同步定案 —— 机制先建全，平台源唯一

owner 要求「清单先同步」，同日就设计稿 RY-204《能力清单同步》的 D1–D4 拍板。§6.2 的
「权威是平台、不从构建产物推导」**维持不变**；三件待定就此定下。

| | 定案 |
|---|---|
| D1 数据源 | **平台源是唯一正式源。** 同步机制本期建全（存储、同步器、接口、界面），数据源可插拔；平台端点（vxture-platform#339）就位之前状态为 `unavailable`，界面如实说。**不随包带 Runos 预置台账快照**：那要推翻 §6.2、把私有的 Runos 仓内容带进公开的仓与安装包，且台账只有技能（877 条），与生产 886 行本就不一致 |
| D2 界面位置（§6.2 待定 1） | 设置 › 能力平台分区新增一格「Runos 清单」，与技能、工具、组件并列；首页不放 |
| D3 刷新时机（§6.2 待定 2） | 登录后取一次；窗口重获焦点时距上次成功 ≥ 6 小时再取；手动随时取。平台给版本 / ETag 就用它，不给就全量取后按内容哈希判变化；取不到保留上一份并标 `stale` |
| D4 对照标记（§6.2 待定 3） | 标「本机可运行」，只按能核对的规则：技能以预置层 `<owner>.<技能名>` 对 `capabilityId`（2026-09-15 实测随包 232 条中 108 条对得上）；连接器只认显式对照表（初始为空 —— 本机 `microsoft.markitdown` 与 Runos `markitdown.document-to-markdown` 不同名，不按名字猜）；执行器恒为仅云端 |

一次同步五步，缺一步不落盘：取（会话读、分页到底）→ 核（条数等于这一次的 `total`、必填
齐、id 不重复）→ 落（整份原子替换）→ 记（来源、时间、版本或内容哈希）→ 比（新增 / 下线 /
变更）。

落点：`apps/local-host/src/capability-catalog.ts`；接口 `GET /capabilities/catalog`、
`POST /capabilities/catalog/refresh`（RY-102）；`<dataDir>/capabilities/catalog.json`，机器级、
不加密、不随工作区切换（Runos 目录不按租户过滤，授权主体是调用它的产品）。**平台端点地址
不预设**：没配就是不可用，不去打一个不存在的地址再把 404 翻译成「平台没有」；平台给了地址
后写进默认值，或先经 `RUYIN_RUNOS_CATALOG_PATH` 接上验证。
