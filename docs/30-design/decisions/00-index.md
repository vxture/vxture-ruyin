...
| [ADR-009](ADR-009-capability-relay.md) | 能力通路定案：经业务产品的云端服务中转 | **被 ADR-026 取代** | 2026-08-31 |
...
| [ADR-020](ADR-020-two-capability-planes.md) | 两个能力提供平台：Runos（云端商业能力面，分发与承载）与 Ruyin（本地执行环境）—— 机制对照、接线与对 ADR-018 的修正；§6.1 追加：预置台账权威归 Runos，取字节的时机是构建时；§6.2 追加：本机保有与平台 Runos 能力目录一致的能力**清单**（清单不是安装；调用路径、不装、不判权益三条不变；§7「不镜像」只对登记册仍成立；取清单的桌面会话端点待平台补，TD-065） | 已接受 | 2026-09-05（§6.1 2026-09-09；§6.2 2026-09-15） |
...
| [ADR-023](ADR-023-product-ui-supply.md) | 产品界面从哪来：契约**钉 sha256 不钉地址**，界面包从产品能力面按摘要取回、校验、按摘要落盘；装载跟项目快照走；界面可选、坏了等于没有界面；契约升 `0.2`、新增 R17；补记：界面在项目侧栏单独一格，有界面时排第一并默认进它 | 已接受 | 2026-09-11 |

| [ADR-024](ADR-024-contract-upgrade.md) | 既有项目跟上产品新版本：**直接升**（生效版本比快照新就升，不用人点）；唯一的保护是新版本删了 / 改窄了项目在用的东西就留在旧版并写明原因；检查器在同构的 contract-schema 里 | 已接受 | 2026-09-12 |

| [ADR-025](ADR-025-capability-routing.md) | 能力调用双机制：**智能体只认 Runos 协议，Ruyin 按配置路由本机 / 云端**，对产品无感；配置三档（只许本机 / 优先本机 / 优先云端），粒度能力 > 工作区 > 租户 > 默认，**默认只许本机**；云端通路未开放前一律收紧到本机、不悄悄出域；界面叫 Runos 须注明「兼容 Runos 协议的本地能力面」；分 P0–P4，本机先行 | 已接受 | 2026-09-15 |

| [ADR-026](ADR-026-capability-and-data-boundary.md) | 能力与数据的边界：**智能体不自建能力面**（取代 ADR-009）；应用能力平台是 Runos；**模型经平台会话中转到 Atlas**；智能体定义由产品提供、针对桌面端打包、平台分发、ruyin 同步与本机存储，按需装到本机；**本机数据不出域、不共享、单人使用**，工作区互通是平台的工作；修订 ADR-012 / 018 / 020 / 023 的来源；§5 收掉为同步预留的旧设计（提议） | 已接受（§5 为提议） | 2026-09-15 |

后续候选（定稿时立 ADR）：Electron vs Tauri（60 §5.1）、
每 Workspace 一库（60 §7.1）、推理传输 ≠ 数据存储（10 §15.2）。

## 与 RY 文档集的对照（2026-09-17，追加；ADR 正文不改）

RY 文档集是权威（`CLAUDE.md`「Design authority」）。与之冲突、或被其推进的 ADR 在这里登记，正文只追加补记：

| ADR | 状态变化 | 去向 |
|---|---|---|
| ADR-009 | 已被 ADR-026 取代（不变） | RY-201 §01、RY-100 §06 |
| ADR-019 | 不变；证据文档 `40-implementation/30` 已退役 | 结论在其 §4-4、RY-101 §06 |
| ADR-021 | **部分被 RY-100 取代**：「对平台的登记只到 OIDC client」→ 运行时实例有自己的身份并向控制面注册（A1–A3）；「会话与设备管理归 accounts」→ Runtime Registry 与同时在线数（A9 / A10）；§4 表的权益读取路径是旧现状 | RY-100 §03 / §08；现状与迁移 RY-103 |
| ADR-026 | §2 第 3 条「ruyin 不发模型回合、模型由产品直接对接 Atlas」**被 RY-100 取代**：循环在本地（ADR-002），模型回合由运行时内核经网关发起，推理上下文是不出域的唯一例外（A7 / A8 / A17）；其余各条不变 | RY-100 §05 / §06 / §07 |
| ADR-003 | 不变；离线宽限的目标态分两层 | RY-100 A13、RY-202 §05 |
| ADR-020 §6.2 | 不变 | RY-204 |
