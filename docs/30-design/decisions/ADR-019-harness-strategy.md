...
- 状态：**已接受并结案（owner 2026-09-06）**：提议 1–3 接受；§4 的三个决策点见 §4；
  **探针已给出结论，owner 定「不换内核」（§4-4，出路 c）** —— 走 §3.1，广度按 ADR-018
  从接缝引入。分析见 `../../40-implementation/30-harness-dsh-spike.md`。（owner 原问：「我们的 harness 是受挫的，有没有官方或优秀
  开源可以借鉴或使用的」）
- 日期：2026-09-05
...
## 4. 已定（owner 2026-09-05）

1. **约束 D 改为条件约束。** 内核今天宿主无关（`runtime-core/src` 零 `node:` 导入），
   云运行时不存在（ADR-008）。探针通过则云端同样嵌 dsh headless（它本就是服务器形态），
   D 的含义变为「两端跑同一个引擎」；探针不通过则 D 照旧（内核同构、云端另写）。
   在探针给出结论前，**内核不引入任何 Node 绑定**。
2. **两周探针批准。** 一人、时间盒 2026-09-06 → 2026-09-19；分支 `spike/dsh-engine`
   不进 main；判据与计划见 `30-harness-dsh-spike.md` §6：C1–C7 全过、§2.3 三条出路
   之一被证明可行且写明代价、体积 / 启动 / 内存增量的数字给 owner 看过再定阈值。
3. **「受挫」按广度理解。** owner 没有点名别的卡点；能力平台（ADR-018）已经在补广度，
   探针答的是「引擎换不换」这一问。若期间发现具体卡点，记进探针报告。
4. **不换内核（owner 2026-09-06，探针结论出来之后）。** 探针在第一周就答了：判据 **C7
   （硬底线不可绕）在 dsh 里不成立** —— `tools/execute` 在所有 guard 之后运行且拿到可变
   的 `exec`，dsh 到 dispatch 才按 `exec.name` 取工具体，放行的 `read_notes` 可被改名成
   `send_report`（external_send）执行而审批被问 0 次。根因：**dsh 的扩展模型假定进程内
   插件可信，Ruyin 的闸门假定它们不可信**；所有绕法都要进程内注册的代码，而那正是
   dsh 的插件生态本身。取 §9.3 的**出路 c**：内核保留，走 §3.1，广度按 ADR-018 从接缝
   （技能 + 进程外 MCP 工具）引入。约束 D 随之回到原样（内核同构、云端另写），
   §4-1 的条件分支不再触发。证据与逐项账目：`40-implementation/30-harness-dsh-spike.md`
   §9，分支 `spike/dsh-engine`（不进 main，作为证据保留）。
...
分析与探针账目所在的 `40-implementation/30-harness-dsh-spike.md` 已退役（原文在 git 历史）。本 ADR §4-4 是结论的权威，两条通用判据（来源不能从日志推、闸门必须是最后一站）在 RY-101 §06；分支 `spike/dsh-engine` 仍作证据保留。
