...
## 补记（2026-09-17，RY-100 之后）

RY-100《顶层架构》确立后，本 ADR 有两处被推进、一处是旧现状，正文不改：

- **「登记只到 OIDC client 这一层」→ 运行时实例有自己的身份。** RUYIN 是运行时节点（A1），每个实例向控制面注册（公钥 + 平台 + 版本 + 能力清单）换实例 id 与实例凭据（A2），并作为自己的 OIDC 客户端登录（A3）。§0「环境不是产品」这个判断不变——实例登记的是 Runtime Registry，不是产品目录；§6「entitlement 项 ≠ 产品」也不变。
- **「会话与设备管理归 accounts」→ Runtime Registry。** 同时在线数按用户限 1 台、团队席位另一维度、超限让用户决定下线哪台（A10）；在线 = 事件通道连接在，撤销秒级（A9）。「设备」这个词作废（RY-103 §06 D11）。
- **§4 表的「权益读取（C2 `/platform/entitlements`）」是旧现状。** 现行路径是 console-bff 会话（RY-102 §03、RY-103 §02）；目标态是 Runtime Contract 的下发（RY-100 §02）。
- `20-specs/10-product-strategy.md` 已退役；§1 引用的定义现在在 RY-100 §01 / RY-501。
