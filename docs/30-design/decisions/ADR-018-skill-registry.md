...
### 7.5 uv 缓存落到数据目录，随包那份只是种子（2026-09-14）

§7.2 第 1 条定的随包 uv 引导件是 `uv.exe` + CPython + 预取好的缓存三件。实施时
`uvxPlan()` 把 `UV_CACHE_DIR` 直接指向随包的 `resources/uv/cache` —— 而 uvx 每次都
往缓存里写（临时环境、解析结果、锁文件），一次冒烟一万多个文件。装到 Program Files
这类非提权不可写的位置（nsis 允许用户改安装目录），首次用 Python 形态的服务器就在
`--offline` 下失败，没有回退；CI 在可写工作区里冒烟，永远看不到（TD-062）。

改法不动 §7.2 的决定，只改缓存落在哪：

- 随包的 `resources/uv/cache` 从此只是**种子**；可写的缓存在 `<dataDir>/tools/uv-cache`，
  与 `UV_TOOL_DIR` 同一种安排。启动计划上多一个 `prepare()`，起进程之前首次把种子种
  过去（跳过软链、重置权限、临时目录原子改名、以 `pythonRuntime` + 全部 uvx「包==版本」
  为种子键，安装包升级即重种）。`plan()` 仍是纯函数。
- 随包的 CPython **保持只读**：实测 uv 检视解释器的临时文件也写在缓存目录里。
- `pack.mjs` 多一条断言：冒烟前后 `resources/` 一个字节都不该变。它把「装到只读位置起不
  起得来」换成了一个可写工作区也能问的问题。

代价：每个安装多占约 200 MB 磁盘，首次用 Python 形态工具多等几秒。

补记（同日）：快照那条断言证明的是「冒烟没往包里写」，证明不了「包写不了时也起得来」——
探一下可写再分岔的代码、只在首次种子时才碰包的路径，快照一概看不见。所以 `pack.mjs`
在第一轮之后再冒一次，条件照 Program Files 那台机器摆：整棵解包树对当前用户拒绝写入
（Windows 走 ACL 拒绝项，目录只读位挡不住建文件），数据目录换成空的临时目录让种子从只读
的包里重种；壳照样 OK、uvx 照样 ok、种子落在临时目录之下才算过。拒绝之后先探一次真假，
探不出只读的演练不算数；做完无论成败都恢复。

### 2026-09-18 补记（TD-042 ②）：Python 半边改为按需获取，§7.2 第 1 条就此作废

owner 2026-09-17 给 TD-042 的定性是：**剔除是对的，不是将就** —— 原话「安装包小一些，
租户按照需求，安装必要环境」。§7.2 第 1 条（uv + uv 托管的 CPython + 预取好的 wheel
缓存随安装包走）与这个定性正面冲突，**作废，不再是这份 ADR 的有效决定**；上面那条
2026-09-06 的 TD-062 补记（缓存落在哪）随之只剩历史价值。

现在的安排：

- **uv 是一条普通的组件**（`components[]` 里的 `python.uv`），走本 ADR §7.2 已有的获取
  通道：https + 闭合白名单 + 不跟跳 + 长度 + sha256 + 解压护栏。sha256 就是原先钉在
  `pythonRuntime.uv` 上的那一条，2026-09-18 真取回来复核过，逐位相同。
- **CPython 与 wheel 由守护进程在用户点下那一次装好**（`apps/local-host/src/python-runtime.ts`）：
  `uv python install` + 用**运行时那条一模一样的命令**预热，最后在一个空的 `UV_TOOL_DIR`
  里 `--offline` 真起一次 —— 这两条是原先构建脚本在构建机上踩出来的，原样搬到了本机。
  全部落在 `<dataDir>/tools/`，所以**包里不再有只读的那一份**，TD-062 那条缝随之消失。
- **我们自己钉 sha256 的只有 uv 那 16,989,876 字节**；CPython 与 wheel 是 uv 按上游校验和
  自核的。这是**委托，不是省略** —— 写在清单里，也显示在界面上。气隙机器这条路仍然
  走不通，要等离线侧载包（TD-042 ③）。
- **uvx 形态不再可能是 `default` 档**：包里一个字节都没有它们。`lint:skill-manifest` 的
  第 1 条从「要满足三个条件」改成「不成立」，而不是被删掉 —— 删掉的话，下一个人把某条
  uvx 挂回默认档时不会有任何东西反对他。`haris-musa.excel-mcp-server` 与
  `vivekvells.mcp-pandoc` 落到 `acquire-on-demand`。
- **`packaged-smoke` 的判据翻了个面**：刚装完的机器上 uvx 自检**只能**是
  `python runtime not installed`，而且安装目录里不许出现 `resources/uv`。报了别的，
  说明 uv 又进包了，或者冒烟捡到了构建机上别处的 uv。

下载的口径一个字不松：启动时、刷新时、任务要工具时一律不下载（§7.2 的硬规则），
装 Python 半边只发生在 `POST /python-runtime/provision` —— 用户点下的那一次。

### 2026-09-19 补记（TD-042 ②）：运行环境重新随包 —— 装完就不该有起不来的能力

owner 2026-09-19 定，原话：「软件安装以后必须保证所有技能运行环境齐备，差异在与能力
是否启用，不能无法启用。能够接受安装包大一些。」

这是对上面那条 2026-09-18 补记的**改判**，也是对 2026-09-17「剔除是对的」那条定性的
改判。两条都留着不删 —— 它们各自那天是对的，改的是权衡的那一端：

- 2026-09-17/18 权衡的是**安装包体积**（小 219 MB），代价是首次用 Python 形态要点一次、
  联一次网；
- 2026-09-19 权衡的是**装完即可用**：一个「装好了但起不来、要你自己再点一次」的能力，
  在真机上被 owner 遇到的样子是「点安装失败，提示重试，仍然失败」。用户那一侧分不清
  「这一版不带它」与「它坏了」，而这两句话该有的反应完全不同。

现在的安排（回到 §7.2 第 1 条，但**不恢复**它的原文，按下面这份写）：

- **uv + uv 托管的 CPython + 预热好的 wheel 缓存随安装包走**（`resources/uv`，约 219 MB），
  `seed-uv-cache.mjs` 回到构建流程。组件 `python.uv` 从清单里删掉 —— 包里已经有它了，
  留着一条永远不会被点的获取项，读起来像一件还没做的事。
- **`python-runtime.ts` 连同 `GET` / `DELETE /python-runtime`、`POST /python-runtime/provision`
  / `…/cancel` 一并删除**（RY-102 同轮更新）。按需装那一路上的每一样东西都删干净，
  不留半条 —— 留着的话，下一个人读到的是两套并存的说法。
- **可写性那条缝仍然按 TD-062 的办法处理**：包里的缓存是只读的一份，起进程之前种到
  `<dataDir>/tools/uv-cache`（`LaunchSpec.prepare()`），**算计划不种**（列表页也会算计划）。
- **`packaged-smoke` 的判据再翻回来**：刚装完的机器上 uvx 自检**必须**是 ok。
- **`lint:skill-manifest` 第 1 条也翻回来**：uvx 形态可以是 `default`，条件是随包 + 不下载
  任何字节 + 开箱即起；`haris-musa.excel-mcp-server` 回到默认档。

同日一并处理的两条，理由不同，不要读成同一件事：`vivekvells.mcp-pandoc` 与
`ihor-sokoliuk.mcp-searxng` 移到 `refused`（owner 2026-09-19）。它们不是「装不上」，
是**装上也没有意义**：pandoc 要本机另装 pandoc.exe，searxng 要用户自己有一个 SearXNG
实例 —— 两条都不是安装包能负责的事，而按上面这条新口径，随包的档位里不许有这种条目。

界面上随之要说的两句话（owner 同日）：能力平台的「本机技能」与「本机工具」两层各自
说清**有几条用不上、为什么** —— 按原因分组，不是给一个总数。装完之后这两句话本该是
空的；它们的用处正是**在不是空的那天说出来**，而不是让用户一行行点开去数。
