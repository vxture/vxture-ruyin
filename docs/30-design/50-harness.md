...
**实现口径（TD-050，2026-09-07）**：用户策略这一层此前**永远是空的** —— 三层
合成的代码一直在，一致性套件也测过它，但 harness 那一行恒传 `undefined`，因为
没有存储。现在它有了，同时有两处偏离上面这张图，写在这里而不是留给下一个人去
对代码：

- **是「每个项目各自的」，不是上面写的「Workspace / 全局」。** 授权本来就以项目
  为边界（ADR-005 第 4 条），而「在这个项目里我允许它写文件」和「在所有项目里都
  允许」差得很远：一个全局开关会让用户在低风险项目里做的放宽，悄悄跟到他下一个
  碰真实客户资料的项目里去。存储落在项目库里，与授权目录同层。
- **底线之下的放宽在写入时就被拒**，不只是在合成时被压回去。两者对闸门的结果
  一样，对用户不一样：一条存下来却永不生效的记录，会让他以为自己关掉了那道确认，
  而每次仍然被问 —— 那比当场拒绝更糟。API 上是 403 `POLICY_DENIED`，界面把守护
  进程的原话照原样显示（它写清了还能改到哪儿）。

下面这张底线表里，**只有 external_send 这一行有实现**。`delete` 在契约的
`ToolCategory` 里根本不存在（六个类别：local_read / local_write / query /
generate / export / external_send），所以那一行现在没有可落的对象；
`confirm: human` 的状态转换由状态机自己守（`transitionBusinessState`），不走
Tool Gate。这两行留在表里是**设计意图**，不是已实现的保障 —— 分不清这两者，
读表的人会以为删除操作已经被看住了。
...
一致性测试套件以此清单为骨架。**已实现（2026-09-01）**：
`packages/runtime-core/src/conformance.ts` 导出 `runConformance(input)`。

它**不是一批测试文件，是一个可以跑在任何 ports 上的套件** —— 不依赖任何测试
框架（依赖 node:test 就等于宣布只有 Node 宿主能验一致性，而那正好否定它的用途），
返回结构化结果，各宿主用自己的 runner 包一层。契约由调用方给，检查断言的全是
契约无关的语义。

当前两套 ports 各跑一遍：runtime-core 的内存实现、local-host 的 SQLite 实现。
**跑两遍才有意义** —— 只在一边跑，验的还是那一边。Cloud Runtime 接的是第三套。

七条都反证过抓得到：改 `MAX_REVISIONS` → C6 报「有界=false」；删
`HARD_FLOOR.external_send` → C7 报违规。**C7 的第一版是空过的**：它只遍历契约
自带的工具，而夹具契约一个 external_send 都没有，于是把底线整条删掉它照样绿。
改为用**合成工具**验 —— 底线是内核的不变量，与任何契约无关。
