...
## 7.1.1 版本号约定（owner 2026-09-18）

**测试版带 `-beta.N` 后缀，毕业为正式版时去掉：**

```
beta    1.2.0-beta.1 → 1.2.0-beta.2 → 1.2.0-beta.3
stable                                    ↓ 毕业
                                        1.2.0
```

**这不是风格问题。** 更新检查按 semver 比，而 `1.2.0-beta.3 < 1.2.0` —— 有后缀，测试版
用户在正式版出来时会被正确地提示升级；没有后缀，两条渠道的同一个版本号对应**两份不同的
字节**，客户端分不出谁新谁旧。允许的形状与包那条守卫一致：`X.Y.Z` 或
`X.Y.Z-(alpha|beta).N`（owner 2026-09-03：三种足够，不要 rc）。

版本号写在**三处**：`apps/shell/package.json`（安装包文件名与 tag 比对用它）、
`apps/local-host/package.json`、`apps/local-host/src/main.ts` 里手写的 `const VERSION`
（守护进程自报、检查更新时拿它跟 feed 比）。三处漂开的后果是安静的 —— 安装包 0.2.0 而
守护进程自报 0.1.0，于是**每一次检查更新都说「有新版本」**，装上还是有。
...
对应哪个 beta tag。要「同一份字节毕业」得先有可复现构建，那是另一个工程。

## 7.1.2 用户那一侧不叫「渠道」（owner 2026-09-18）

界面上是**偏好设置里的一个开关：「抢先体验新功能」**，默认关。「渠道」是发布侧的词，而
用户要做的决定只有一个 —— 想不想早点用上新功能，代价是更可能遇到问题。不弹窗、不在首页
推：想尝鲜的人找得到，不想的人碰不到。

偏好落在 `<dataDir>/updates.json`，接口是 `GET` / `PUT /updates/channel`（RY-102 §03）。
`GET /updates/check` 据此拼出渠道目录 —— **检查哪个渠道就下哪个渠道**。

**关掉开关时有一句必须说**：用户装着 `1.3.0-beta.2`、切回正式版（当前 `1.2.0`），检查更新
会回「已是最新」（本机版本号更高）—— 他会以为自己回到了正式版，其实还跑在测试构建上。
所以关掉之后立刻查一次，并把那句话与下载地址一起摆出来（地址来自刚校验过的那份 feed，
界面不写死）。**不做自动降级**：降级要卸载重装，风险比收益大。软件更新那一块里只读的
「更新渠道」行照旧留着 —— 那说的是「你现在装着什么」，与「你要什么」是两件事。
...
**两个渠道都在这台主机上，且都公开可读**（owner 2026-09-18 定）。客户端的更新检查按渠道
拼地址 —— 检查哪个渠道就下哪个渠道，两者不可能不一致；把 beta 留在别处就成了两套地址、
两套可达性、两种失败模式，而内测用户同样在境内。代价照实记：**任何知道 `…/beta/Ruyin-Setup-latest.exe` 的人
都能下到测试版**，这不是漏洞，是「公开下载主机」的定义。要让测试版只给受邀的人，公共读
做不到半公开，得另开私有桶配签名 URL（而签名 URL 有时效，当不了固定地址）—— 那是另一个决定。

**下载地址就是发布地址**（下载主机的渠道目录，见上面的目录树）：
