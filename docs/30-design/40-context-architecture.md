...
>
> **2026-09-17 对齐说明。** 01 / 02 / 03 已退役；上位设计在 RY-100 §06（能力路由两问：需要密钥 → 云端，碰用户数据 → 本机）与 §07（数据边界：不出域的唯一例外是推理上下文）。本文件 §3–§7 的机制（来源绑定、连接器、索引、最小化选择、传输门）被 `connector-fs.ts`、`connector-mcp.ts`、`fts.ts`、`context-budget.ts`、`harness.ts` 直接引用，仍是工程权威；**§8 有意不做、§9 已被 RY 取代**，两节各自开头有说明。
...
**预算这一步的实现口径（TD-044，2026-09-07）**：预算的单位是**字节**，而不是
token —— 这里把它写清楚，因为「token budget」这四个字会让人以为运行时在数 token。

它没有在数。token 数要由**分词器**给，而分词器属于模型那一侧；本地运行时没有，
也不该为了估算把某个模型的分词表抄一份进来 —— 抄进来的那份会随模型更迭漂开，
而**漂开的估算读起来和准的一样**。字节是我们真实掌握的量（就是要发出去的那些
字节），与 token 大致成比例，且永远不会低估。

三条随之而来的口径：

- **裁的是条目，不截断某一条**。截断会让成果里的引用指向原文中并不存在的半句话，
  而「结论可回溯到原文」是本产品的立身之本之一（§6.2）。宁可少一条完整的。
- **按排位轮转取**（先给每一类第一条，再回头给第二条）。否则 `input_types` 里排
  第一的那一类会吃光预算，后面的类型一条都拿不到 —— 而任务声明它们，正是因为
  它需要它们。预算不够时缺的应该是「某一类的第二条」，不是「某一类全没有」。
- **两处会超预算放行并记一笔**：`required: true` 类型的第一条，以及「一条都没
  选上」的兜底。否则会出现用户无法处理的失败 —— 资料就在那儿，只是太大，而 §6.3
  那句「明确报告缺失项」会把「预算太小」说成「你没绑目录」。

缺省 800 KB；宿主可覆盖（桌面侧 `RUYIN_CONTEXT_BUDGET_KB`）。**内核自己不读环境
变量**——内核宿主无关（ADR-008），云端那一侧没有这个进程的环境，所以这个数由宿主
经 `RuntimePorts.contextBudgetBytes` 传入。
...
> **有意不做（ADR-026 §5，TD-043 standing；2026-09-17 复核）。** ruyin 不同步智能体数据：本机数据不出域、不共享、单人使用（RY-100 §07，I2）；工作区之间的互通是平台的工作。本节保留作设计记录，不是待办。契约 §14 `sync` 键只收 `local_only`。
...
> **已被 RY 取代（2026-09-17）。** 本节 §9.1 的身份链（浏览器 OAuth / 设备码 → 设备绑定 → 短期 access token + refresh）是 2026-09 之前的旧路径，从未成为现状。**现状**：console-bff 原生反向握手，本机只持不透明会话号 `rpsid`（RY-102 §03、RY-103 §02）。**目标**：三种正交主体（用户 · 运行时实例 · 产品）、实例凭据 DPoP 绑定、RUYIN 是自己的 OIDC 客户端、常驻委托按产品 × 项目（RY-100 §03，A2 / A3 / A14）；在线与并发归 Runtime Registry（RY-100 §08）。§9.2 的离线语义按 RY-100 A13 分两层：运行时只要账号在永远可进、永远可导出；产品到期各自算、宽限期由产品声明。§9.2 里 2026-09-01「导出是鉴权操作，不是逃生口」的裁定**已被 RY-100 §07 / A11 取代**：任何状态下本地数据都可导出（未登录、离线、权益过期、被吊销都不例外）——现行实现仍要求会话（`409 WORKSPACE_REQUIRED`），是现状与目标的差距，随 RY-103 迁移收敛。§9.3 四层交汇的形状仍成立，对应 RY-100 §04 三个互不覆盖的授权权威（权益 · 配额 · 策略）。
