...
| T13 | 打包与更新 | electron-builder；**MVP 不做自动更新**（2026-09-02 owner 定，TD-021）——检查保留，下载与安装由用户在浏览器里自己完成；electron-updater 已整段移除。企业环境提供离线 MSI；产品包走 03-A §18 独立通道 | — | 信创路线启动时补充 Linux 打包；自动更新等签名证书（TD-001，现为 standing）或一个「接受关掉签名校验」的决定 |
...
files/ 区 → 同密钥体系加密（用的就是上面这把项目密钥）
```

**文件区的实现口径（TD-041，2026-09-07）**：

参考资料**默认仍然不进数据目录** —— 按目录授权从用户自己的位置读，抽出的内容进
`project.db`。文件区是一条**用户显式发起**的旁路：他把一份原件「收进项目」，从此
它与他自己那份无关。存在的理由是一句话 —— 半年后有人问「第 17 条里这个数字是哪
来的」，而引用指向的那个路径已经不在了。

磁盘格式（`apps/local-host/src/file-store.ts` 的头注释是权威）：

```text
<projectDir>/files/<hash 前两位>/<明文的 sha256>

  "RYF1" | chunkSize(u32) | plaintextSize(u64)
  然后重复： nonce(12) | tag(16) | ciphertext(≤ chunkSize)
```

四条口径，每条都有具体理由：

- **逐块加密，块大小 4 MiB**。一份 2 GB 的标书整份读进内存再加密，会在用户点
  「收进项目」的那一刻吃掉 2 GB —— 那是他自己的电脑（与 TD-046 同一条道理）。
- **每块自己的随机 nonce，块序号绑进 AAD**。只加密不绑序号的话，拿到文件的人可以
  把块换个顺序拼回去，而每一块的 tag 都仍然对得上。
- **文件名是明文的 sha256，读回时重算比对**。内容寻址顺带带来去重：同一份内容收
  两次，磁盘上只有一份。
- **登记与密文分开**：登记（名字、时间、来源）在 `project.db` 的 `files` 表里，
  多条登记可以指向同一个 hash（同样的字节两个名字是真实存在的情况）；删掉最后一条
  指着它的登记时，那份密文才真的删。少了这个引用计数，删掉一个名字会让另一个名字
  指向一个不存在的文件 —— 而症状要到用户去取原件时才出现。

收进来**必须来自已授权目录**（04 §4.3）：这一步是把文件复制进数据目录，少了那道
校验，一个 POST 就能让守护进程读走机器上任意一个文件。
...
产品 iframe（sandbox 属性 + 独立 origin + CSP）
        │  postMessage
Workspace UI 桥接层（转发，不做裁剪）
        │  携带**产品级凭据**（按 projectId + productId 限定、短时）
Local API  →  **按该项目签下的契约快照裁剪**（裁剪在这里）
```

产品 UI 能做什么由其契约决定，**而裁剪由守护进程执行**（ADR-022，2026-09-10 修订）。

> **原文这里写的是「桥接层按契约裁剪 API 面 + 携带会话 token」。** 改掉的理由：
> 那样守护进程分不出「这次调用来自产品界面」还是「来自工作台自己」，于是
> 「UI 层不可能越出 Tool Gate」这句保障的强度就等于桥那几百行前端代码，审计里也
> 记不下发起方是谁。对照本仓自己的判据 —— **闸门必须是最后一站**
> （`40-implementation/30` §9）。
>
> **沙箱那一层没变，而且不可省**：它挡的是「产品代码直接够到 Local API」。两道门
> 各挡各的，缺一不可。
>
> 会话 token 与产品级凭据**互不通用，两个方向都拒**：只拒一边的话，另一边就是
> 那条没人走过、坏了也看不出来的路。
