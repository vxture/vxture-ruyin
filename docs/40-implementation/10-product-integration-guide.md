...
| `haris-musa.excel-mcp-server` | acquire-on-demand | — | _未探到：本次构建没有 vendored 它（runtime = uvx）_ |
| `vivekvells.mcp-pandoc` | acquire-on-demand | — | _未探到：本次构建没有 vendored 它（runtime = uvx）_ |
...
> **现状（2026-09-11，ADR-022 片四第一批：只读的面）。** 上面是设计基线（SDK 包
> 等第一个真要接的产品，ADR-022 §5 修订）。今天产品界面经桥能调到的，**只有下面
> 这几个**；每一个都在守护进程里按字段挑着给，不是整条记录转手：
>
> | 面 | 桥上的地址 | 给 | 不给 |
> |---|---|---|---|
> | 上下文 | `GET /context` | 绑定的类型、来源、连接器 | 绑定的目录（本机绝对路径）、条目内容 |
> | 项目（基线里叫 `workspace`，ADR-007 起本地容器叫项目） | `GET /project` | 名字、形态、业务阶段、此刻能推进到哪（及是否要人确认） | 工作区 / 租户 id、目录授权、审计 |
> | 任务 | `GET /tasks` | 哪个任务、状态、时间、在等哪一类确认、有没有成果 | 输入与选中的资料、确认的内容（工具参数）、对话、错误原文 |
> | 成果 | `GET /tasks/:id/result` | 内容（产品自己的能力面生成的）、资料条目 id、出处、校验类别与结论 | 条目本身（路径、文件名）、评审意见原文 |
>
> **写的面（第二批，owner 2026-09-11「按推荐走」）。关卡对谁都一样** —— 产品发起的
> 与人在 Runtime 界面里点的走同一套判断，区别只记在审计里（`requestedBy`）：
>
> | 面 | 桥上的地址 | 行为 |
> |---|---|---|
> | 推进业务阶段 | `POST /project/transition { to }` | 不要人确认的：直接推进。**要人确认的（`confirm: human`）：只挂成请求**，Runtime 在产品界面上方钉一张确认卡、并列进「在等我」与系统通知；人点了才推进。「人已确认」这个标记**桥上不收**。挂着时新的请求拒收（不顶替）；`GET /project` 的 `pendingTransition` 告诉产品「提了，人还没点」 |
> | 发起任务 | `POST /tasks { taskId }` | **只按任务名**：资料由 Runtime 按契约挑选，带 `inputs` 直接拒（不是悄悄忽略）。资料确认、工具闸门一道不少 |
>
> **事件投影（第三批）。** 桥挂上时连守护进程的 `GET /bridge/events`（产品级凭据），
> 把每一帧原样转投成 `{ns:"ruyin.bridge", kind:"event", topic, payload}`：
>
> | topic | 什么时候 | payload |
> |---|---|---|
> | `task` | 这个项目里某个任务动了（起来了、落定了、停在等人那一刻） | `{ taskInstance }` |
> | `project` | 这个项目的业务阶段、或挂着的推进请求变了 | `{}` |
>
> **只说什么变了，不说变成了什么** —— 收到之后回头问 `GET /tasks` / `GET /project`，
> 走的还是那几个裁剪过的面。给什么由守护进程定（白名单：只有这个项目、只有这两种；
> 总线上以后新加的事件默认不给），**桥不筛**。流开着时守护进程每写一条、每次心跳都
> 重新认凭据，认不下来就收掉，桥换张凭据重连。
>
> **还没有的**：`objects` —— 运行时里还没有业务对象的存储，不为一个没有数据的面
> 造接口。**检查点只读**：产品只看得到「在等哪一类确认」，呈现与决定永远归 Runtime（§6.3）。
