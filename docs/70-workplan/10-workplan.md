...
- [ ] 推 main → 确认五个 required context 全绿一次
- [ ] apply ruleset：`gh api repos/vxture/vxture-ruyin/rulesets --input docs/50-deployment/rebuild/main-ruleset.json`（TD-002）
- [ ] 仓库 Settings 开启 secret scanning + push protection
- [ ] 确认 org 级 NODE_AUTH_TOKEN 已共享到本仓
- [ ] 本地 hook 接线：`git config core.hooksPath .husky`（每个 clone 一次）
