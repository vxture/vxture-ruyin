...
## W7 · 兑现「承载多个」（下一批，2026-09-07 定，owner 批准）

> **判据：一台机器上同时跑起多个任务时，运行时说得出「谁在跑、花掉多少、占了多少」，
> 并且能拦得住。** 现在这三句一句都说不出来。

**为什么是这一批。** 「在用户自己的机器上承载**多个**业务超级智能体」是定位的一部分
（`30-design/10`），而它是定位里**唯一还没兑现的一片**：核心执行链路（循环、闸门、
人在回路、审计、存储、能力平台、供给）设计与实现是对得上的，缺的集中在这里与云端同步
两块。同步等 Cloud Runtime（W6，平台侧协同），**这一批四件全部不依赖任何外部方**。

这几条此前的处境值得记一句：它们在本页「明确不在 MVP」里被**有意延后**过，但
**没有进技术债登记册** —— 2026-09-07 补登记（TD-043…TD-049 / TD-050）。
一件被决定延后的事，如果只活在一份计划的某一节里，下一个人清点欠账时数不到它。

### 范围（六项）

- [x] **TD-045 · 并发与调度**（2026-09-07 完成）—— 排最前
      现在只有一个去重集合（`TaskRunner.inFlight`）：它保证同一个任务不被跑两遍，
      **不约束同时跑几个**。要的是上限 + 队列 + 优先级。
      **排最前不是因为最急，是因为另外两件以「同时有多个」为前提** —— 预算与资源
      治理在单任务下都度量不出问题。
      判据：同时提交 N 个任务，运行中的数量不超过上限，其余在队里等且状态看得见。
      **已达成**：上限默认 3（`RUYIN_MAX_CONCURRENT_TASKS` 可覆盖）+ 两档优先级
      （用户刚点的 > 启动扫出来的）+ 队里位置报到界面（`排队中 · 第 N`）。
      排队**不进状态机、不落库** —— 队列不持久化，落库的排队状态在下次启动时会是
      一句谎。7 条队列用例 + 7 条调度用例，含两条最容易漏的：取消排队中的要真的
      从队里拿走、失败的任务同样腾出名额。
- [x] **TD-044 · 上下文预算**
      设计说「token budget 内取最小充分集」（`30-design/40` §6.1），实现是
      `MAX_ITEMS_PER_TYPE = 3` —— **按条数裁，不是按预算**。一份大文档整条进上下文。
      判据：给定预算，选择结果随之变化；超预算时**裁的是条目，不是把某一条截断**
      （截断会让引用指向半句话）。可先用字符数近似，但**要明说是近似**。
      **已达成**：`context-budget.ts` 一段纯逻辑，缺省 800 KB，宿主可覆盖
      （`RUYIN_CONTEXT_BUDGET_KB`，要不限得明写 `unlimited`）。单位是**字节**并
      在代码与文档里都直说它近似 token —— token 数得由分词器给，而分词器在模型
      那一侧，抄一份进来的估算会和真在用的模型漂开，且漂开之后读起来和准的一样。
      按**排位轮转**取（先给每类第一条，再回头给第二条），否则第一类吃光预算；
      放不下就**整条不要**，不截断任何一条；必需类型的第一条与「一条都没选上」
      两种情况会**超预算放行并记一笔**，否则会把「预算太小」报成「你没绑目录」。
      9 条预算用例 + 4 条宿主取值用例 + 1 条端到端（证明这个数真的走到了选择）。
- [ ] **TD-046 · 本地资源治理**
      工具子进程（MCP 服务器）与索引都不受任何上限约束。
      判据：至少给这两处各落一个上限，超限时**说得出是谁超的**。
      **那是用户自己的电脑，不是可以随手重启的服务器。**
- [ ] **TD-050 · 用户策略持久化**（可与上面三件并行，改动区不重叠）
      三层合成里唯一一层永远是空的。落持久化 + 界面。
      两条一起守：**硬底线不可被它放松**；**它是每个项目各自的**，不是全局开关。
- [ ] **TD-039 · 数据目录的系统目录拒绝清单**
      眼下靠可写探测与卷检测挡住大部分，但把数据目录指到系统目录仍是用户做得到的坏选择。
- [ ] **TD-041 · 项目的「文件区」**（owner 2026-09-07 确认仍要做）
      设计里有、实现不收原件。**这一条是六件里唯一用户直接看得见的**。

### 不在这一批（各自的理由）

| 事 | 卡在哪 |
|---|---|
| TD-043 同步引擎 | 等 Cloud Runtime，属 W6，平台侧协同 |
| TD-012 / 036 / 037 签名三条 | 卡在**谁来当根证书** —— 是决策，不是编码 |
| TD-038 分发主机 | 等平台 |
| TD-048 契约版本演进 | 等第二个真实产品，否则没有升级对象 |
| TD-049 跨产品协作 | 需求驱动，不预设 |

### 比这一批更靠前的前置，不在本仓

**业务产品的能力面**（产品线）。它不接，任务跑出来的仍是占位输出。
本批做完，本仓这一侧仍然只能说到「就绪待接」—— **这句话不是免责，是验收方式**：
这一批的每一件都必须能在没有真实能力面的情况下被验证，否则它就不该排进来。
...
## 探针 · DeepSeek Harness 能不能当引擎 —— **已结案（2026-09-06，不换内核）**

- 依据：ADR-019（已接受）；计划、判据与进度：`40-implementation/30-harness-dsh-spike.md` §6 / §8。
- 形态：一人、时间盒两周、分支 `spike/dsh-engine` **不进 main**；产出 = 报告 + 可运行的分支。
- 通过判据：C1–C7 对着 dsh 版 Harness 全过；§2.3 三条出路之一被证明可行且写明代价；
  体积 / 启动 / 内存增量的数字给 owner 看过再定阈值。
- **结论（第一周就答了，未用满时间盒）**：判据 C7（硬底线不可绕）在 dsh 里不成立；
  owner 2026-09-06 定**不换内核**（ADR-019 §4-4，出路 c）。ADR-002 / ADR-008 / ADR-011
  **不修订**；约束 D 回到原样。报告与逐项账目：`40-implementation/30-harness-dsh-spike.md`
  §9 / §10；分支 `spike/dsh-engine` 保留为证据，不进 main。
- **后续**：广度按 ADR-018 从接缝引入（技能 + 进程外 MCP 工具，已在做）；
  沙箱台阶 / 作业 / 子代理 / 压缩 / 会话回放这几项**没有**因此拿到，需要时各自立项
  （状态机底座若再评，候选仍是 LangGraph.js，它满足约束 D）。
