...
- [ ] **M1 · 一级供给：契约拉取**（ADR-012）
      走产品能力面 → 过 R1–R13 → 按版本缓存 → 离线沿用。不依赖 Registry，
      不依赖签名信任锚。
- [ ] **M3 · 上下文承载面支持非文本**
      `ContextItem.content` 目前写死为 string，连接器因此只能承载纯文本。
      **MVP 做的是「让内容类型可扩展」这条接缝，不是内建某几种格式** ——
      具体支持哪些格式属技能选型，取决于承载哪个产品，不在本项。
- [ ] **M2 · 产出落盘支持二进制**
      工具执行器目前只写 UTF-8。同样只做接缝：**能把字节写到授权目录内**。
      渲染成什么格式是技能的事（ADR-013），哪些预制渲染技能进包，同样取决于
      产品，不在本项。
- [ ] **M4 · 人在回路的通知**
