...
- [x] **TD-046 · 本地资源治理**
      ...
      **已达成**：`resource-limits.ts` 落五条 —— 同时运行的工具服务器数（6）、
      一条 JSON-RPC 消息的字节数（8 MB）、一次工具结果的字节数（1 MB）、一次索引
      的条数（5000）与字节数（256 MB）。**最要紧的是第二条**：此前 stdout 按行读
      而「一行」没有任何长度限制，一个不吐换行的服务器能把守护进程整个撑爆 ——
      别的几条是「多了慢」，这一条是「一条消息就没了」。
      超限一律说得出是谁：拒绝工具服务器时列出正占着名额的 id（实跑守护进程验过
      那句话），索引跳过时回 `skipped`/`stoppedBy` —— 只回一个条数的话，「一万份
      里索引了五千份」和「这里就只有五千份」是同一个数字。
      15 条用例（取值 7 + 子进程 4 + 索引 4）。**顺带更正登记本身**：它写着下载也
      不受限，核过是错的 —— 获取通道早就按清单里的 size 卡两道。
- [x] **TD-050 · 用户策略持久化**（可与上面三件并行，改动区不重叠）
      三层合成里唯一一层永远是空的。落持久化 + 界面。
      两条一起守：**硬底线不可被它放松**；**它是每个项目各自的**，不是全局开关。
      **已达成**：`tool-policy.ts` + `ProjectStore.putToolPolicy/getToolPolicy`
      （与授权同一个项目库）+ 上下文页的「工具权限」一段。harness 里那行
      `userPolicy: undefined` 换成**每轮重读**的真表 —— 用户可能在任务停在等人那
      一刻去改，改完接着点批准，期待的正是新策略生效。
      两条都守住且各有用例：底线之下的放宽**当场拒绝**（403 POLICY_DENIED，而不是
      存下来再在读的时候忽略 —— 一条永不生效的记录会让他以为自己改成了，那比拒绝
      他更糟）；策略**每个项目各自**。界面每行显示的是三层合成后**实际生效**的值
      与「谁说了算」，不是「用户设过什么」—— 后者回答不了他真正的问题。
      7 条纯逻辑 + 4 条端到端（证明这张表真的走到了闸门）+ 1 条 HTTP + 6 条界面。
- [x] **TD-039 · 数据目录的系统目录拒绝清单**
      眼下靠可写探测与卷检测挡住大部分，但把数据目录指到系统目录仍是用户做得到的坏选择。
      **已达成**：`system-dirs.ts`，接在 `checkTarget` 里、**排在可写探测之前** ——
      这些地方多半是写得进去的：`C:\Windows\Temp` 可写、空间够、同卷，探测一路
      绿灯，然后数据躺在一个下次系统清理就会被扫掉的地方。
      拒 Windows 系统目录 / 程序目录 / **应用自己的安装目录**（卸载会连数据一起
      删，而卸载时用户以为自己在删一个程序）/ ProgramData（主密钥按当前用户封装，
      换个用户登录就解不开 —— 这条用户不可能自己想到）/ 盘符根 / AppData 与主目录
      **本身**（只拒它们自己：默认位置就在 LocalAppData 下面，整棵树拒掉等于拒掉
      自己的缺省）/ 回收站与系统卷信息；POSIX 与 macOS 各一份。
      规则从**传进来的环境**算，不摸 `process` —— 否则占清单绝大部分的 Windows
      规则在 ubuntu 的 CI 上一次都不会被执行，而没被执行的规则坏了和好了长得一样。
      已在用的目录若落在系统位置上**只警告不拦启动**：拒绝启动会让应用变成一个
      既打不开、也没法在界面里把目录改回来的东西 —— 改目录的界面正在它里面。
      14 条清单用例 + 2 条 checkTarget + 1 条端到端 HTTP。
- [x] **TD-041 · 项目的「文件区」**（owner 2026-09-07 确认仍要做）
      设计里有、实现不收原件。**这一条是六件里唯一用户直接看得见的**。
      **已达成**：`file-store.ts` —— 内容寻址（文件名 = 明文 sha256，顺带去重）
      + **逐块加密**（4 MiB 一块，每块自己的随机 nonce，**块序号绑进 AAD**：不绑
      的话把两块换个顺序拼回去，每块的 tag 都仍然对得上）+ 用的是**和
      `project.db` 同一把项目密钥**（§7.3「同密钥体系加密」）。
      登记在 `files` 表，**多条登记可以指向同一个 hash**（同样的字节两个名字是真实
      存在的情况），删掉最后一条指着它的登记时那份密文才真的删 —— 少了这个引用
      计数，删掉一个名字会让另一个名字指向一个不存在的文件，而症状要到用户去取
      原件时才出现。
      收进来**必须来自已授权目录**：这一步是把文件复制进数据目录，少了那道校验，
      一个 POST 就能让守护进程读走机器上任意一个文件。
      界面在「上下文」页，与授权、绑定挨着摆 —— 它们讲的是相反的两件事（指着你
      自己的位置读 / 复制一份进来），分到两个页面去用户会以为是一回事。
      **默认口径不变**：参考资料仍然不进数据目录，文件区是用户显式发起的旁路。
      14 条存储用例（含改字节、换块序、换钥匙、整份掉包四种篡改）+ 2 条 HTTP
      + 9 条界面 + 3 条 API。

- [x] **TD-051 · 云同步目录拒绝**（owner 2026-09-07 定：拦截 + 界面提醒）
      做 TD-039 时提出、当时按题目范围留到了外面。两条后果都不可逆，而且用户看不
      出来：同步客户端会在加密库**正被使用时**改动它的文件（被换掉半个 WAL 就是整个
      库读不出来）；它还会把契约、审计链、文件区里的原件整份传上第三方网盘 —— 而
      「数据不出本机」是这个产品的立身之本之一。
      **已达成**：`cloud-sync.ts` 三道认法（客户端声明的根目录 / 同步工具留的标记 /
      路径里的已知名字），排在可写探测之前 —— 一个 OneDrive 文件夹当然是可写的。
      第三道会误伤一个手工建的同名文件夹，**这是有意接受的代价**：误伤是换个目录，
      漏判是库被改坏或数据被上传，两者不对称。
      换目录的弹窗里**在选之前**就写明不要选云同步目录：选完再拒也拦得住，但那时
      用户已经挑了一个他觉得很合理的位置。
      **同日第二轮**（owner：拦截轻量化、只拦认得出的；导出与收原件也一起拦；
      **第一位是提醒**）：
      - 一个探测器 `cloudSyncHit`，**三句不同的话**。三处拒绝的理由并不相同 ——
        数据目录是「活的库会被改坏」，导出是「数据出本机」，收原件是「收进来的
        可能只是占位存根，而那种失败和成功长得一模一样」。共用一句会让其中两处的
        解释是错的，而一句解释错了的拒绝比没有解释更让人无从下手。
      - 导出接在 `writeArtifact` —— 运行时往用户自己的位置写东西的**唯一出口**
        （导出项目记录、`export_result`、`write_document`、壳排的 PDF 全走它），
        接在这里「成果不会被悄悄传上云」才对所有出口成立。收原件接在
        `POST /projects/:id/files`。
      - **界面上的提醒升为警示样式、排到选目录之前**，并明说「常见的几家会被自动
        拒绝，**但挂成盘符的那些认不出来**，请你自己确认」。认不出来的拦不住 ——
        不说这句，用户会以为「没被拒就是安全的」。
      14 条清单用例 + 1 条导出落盘 + 1 条收原件 HTTP。
...
| 步骤 | 现状 | 断点 |
|---|---|---|
| 装上 | ⚠️ | **不签名（TD-001 已定不采购）**。用词更正：不是「装不上」——SmartScreen 是**警告**不是封锁，用户点「更多信息 → 仍要运行」即可装。代价是首次安装有一道劝退对话框，且**买了证书也不会立刻消除它**（EV 的 SmartScreen 特权已于 2024-03 被微软取消，EV/OV 一律靠下载量累积信誉）。<br>**更正（2026-09-15，随 TD-001 补记）**：「警告不是封锁」只对 SmartScreen 这一层成立。开着 Smart App Control 的机器上，下载来的未签名安装包是**直接封锁**，双击没有反应、没有「仍要运行」；新装的 Windows 11 默认开启或处于评估状态。这类用户在不签名的前提下确实「装不上」，逐台绕过（解除锁定 / `Unblock-File`）不是产品解法 |
| 登录、看到已订阅的产品 | ✅ | |
...
| ~~代码签名证书（TD-001）~~ | ~~采购~~ | **已移出外部依赖（2026-09-02）**：owner 定不采购，TD-001 转 standing。它本来就不是「MVP 能不能被使用」的闸门——SmartScreen 只警告不封锁。<br>**更正（2026-09-15，随 TD-001 补记）**：这条理由只有一半成立。SmartScreen 确实只警告，但开着 Smart App Control 的机器上，下载来的未签名安装包被直接封锁，那部分用户装不上。「不是闸门」的判断因此需要 owner 按 TD-001 补记列出的三条解法重判；本表不替 owner 改结论 |
