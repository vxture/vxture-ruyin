...
> **命名（2026-09-03，owner 定）：品牌 = 产品 = RUYIN（大写），标语 Intelligent Workbench。**
> 产品内展示的名字只有 RUYIN：界面字标、文档标题、登录与关于页；字标旁不再并列中文名。
> 安装包、开始菜单与可执行文件保持首字母大写的 Ruyin（Ruyin-Setup-x.y.z.exe）。
> 小写 ruyin 只用于标识符（npm scope、环境变量、路径、日志前缀）。
> 文档正文里的「Ruyin（如影）」是写作用法，不改。
