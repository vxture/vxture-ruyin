...
function startDaemon(): Electron.UtilityProcess {
  const child = utilityProcess.fork(daemonEntry, [], {
    ...
    env: {
      ...
      // 预置的 MCP 服务器（vendored 的 node 包）随安装包在 resources/tools。
      ...(app.isPackaged ? { RUYIN_TOOLS_DIR: join(process.resourcesPath, "tools") } : {}),
      // 随包的 Python 引导件在 resources/uv（守护进程按 tools 目录的兄弟位置找，
      // 见 tool-servers.ts 的 uvHome()）。
