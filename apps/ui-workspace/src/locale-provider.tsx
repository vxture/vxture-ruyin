 ...
 * 「重启以应用语言」是上个时代的事。
 *
 * 壳那一侧（原生对话框、系统通知、搬家那一屏）读的是守护进程写下的那一份：
 * 切语言时这里顺手 `PUT /system/language`，守护进程落进 `location.json` ——
 * 那是唯一一份两个进程都读得到、且在开任何库之前就读得到的文件。
 *
 * **写回失败不拦人**：界面这一侧已经切好了，壳下次启动跟上就是。为一次写不
 * 进去的偏好把切换挡回去，是拿一件小事换一件更小的事。
 */
