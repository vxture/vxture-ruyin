...
export type RuntimeEvent =
  ...
  | { kind: "pending" }
  /**
   * 某个项目的业务阶段、或产品提出的推进请求变了（ADR-022 片四）。产品界面经桥
   * 订得到**自己那个项目**的这一种（bridge-events.ts）。
   */
  | { kind: "project"; projectId: string }
  /**
   * 某个获取通道的组件动了（开始取、进度、成功、失败、被移除）。
   * **只说什么变了，不带数值** —— 进度是连续的，事件不该是；界面收到之后自己去
   * `GET /components` 取那一份。发布方节流到约每秒一次。
   */
  | { kind: "component" }
  /**
  ...
  /** 请壳打开日志目录（TD-066）。与上一条同理：路径由壳自己算，事件不带数据。 */
