...
export interface LocationFile {
  ...
  dataDir?: string;
  /**
   * 界面语言（owner 2026-09-17）。**放在这里是因为这是唯一一份壳与守护进程
   * 都读得到、而且在开任何库之前就读得到的文件。**
   *
   * 壳需要它：原生对话框、系统通知、搬家那一屏都由壳出，而搬家那一屏出现在
   * 守护进程起来之前 —— 那时问不到任何接口。界面自己另有一份 localStorage
   * 缓存（切了当场生效，不必等一次往返），这里这份是给壳看的。
   *
   * 没有这一条时，壳落到操作系统的语言。
   */
  language?: string;
