 ...
 *
 * 这两条判据 2026-09-18 撤过一次（那天 uv 不随包，包里没有种子，这条缝不存在），
 * 2026-09-19 随 owner 的改判搬回来。**撤得对、搬回来也对** —— 判据跟着包里到底有
 * 什么走，而不是跟着上一版的代码走。
 *
 * @param {{ smokeOut: string, dataDir: string, expectUvx?: boolean }} x
 * @returns {{ ok: true, detail: string } | { ok: false, message: string }}
 */
export function judgeReadOnlySmoke({ smokeOut, dataDir, expectUvx = true }) {
  ...
  if (!expectUvx) return { ok: true, detail: "壳 OK（这一轮没拉随包工具，uvx 与种子不在判据里）" };
